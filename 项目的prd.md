《读者之旅 Reader’s Journey》PRD（离线纯前端 MVP，v1.6）
...
### 3.1 世界页（横版探索）
1. 默认首页为“旅者小屋”横版世界。
2. 角色可左右行走（触屏按钮与键盘双支持）。
3. 交互点映射：录入台、属性镜、藏书阁、信使鸽、工坊。
4. 靠近交互点后点击“交互”，进入对应功能模块（分享点直接打开证书 Sheet）。
...
### 3.3 面板页（主位）
1. 展示等级与经验摘要。
2. 展示六维属性全量（图标、数值动画、段位、属性能量条）。
3. 展示技能星图分布（点亮状态、自动装备 3 槽）。
4. 展示成就摘要，并支持弹层查看全部。

### 3.4 藏书阁页
1. 展示“卷轴书架”（在读 + 已完成）。
2. 点击卷轴展开，再进入“书卷详情 Sheet”。
3. 可编辑阅读进度（支持校正回退确认）。
4. 可新增/编辑/删除阅读感触时间线。
5. 进度上升时按原规则结算经验、属性、技能、成就。

### 3.5 分享（右上角图标入口）
1. 任意页面点右上角图标打开“星图证书”Sheet。
2. 生成角色卡信息（昵称、等级、优势属性、技能摘要、邀请码）。
3. 支持复制邀请码与分享文案。
...
1. 优先软件感，不做网页感。
2. 可读性优先，主区允许纵向滚动；详情统一走底部弹层（sheet）。
3. 动效轻量且即时，不影响操作效率。
4. BGM 使用长音环境氛围，支持开关与跨场景淡入淡出。
5. 素材仅使用开源可商用或 AI 生成并记录来源与许可。

## 7. 技术架构（本版本）

1. 纯前端：HTML + CSS + JavaScript（PWA）。
2. 世界场景引擎：Phaser 3（离线 vendor 打包）。
3. 支持离线缓存：Service Worker。
4. 不依赖后端数据库与定时任务。
5. 不接入登录系统。
...
3. 主页面无半截文本，主内容滚动与 Sheet 详情均可正常阅读。
4. 数据重启后可恢复。
